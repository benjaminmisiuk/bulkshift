bulkshift <-
    function(shift,            #dataset being corrected
            ...
            if(shift.method == "lm"){
                ...
                if(err.plots == "TRUE") {
                    if(length(preds)==1){
                        ...
                        } else warning("Too many dimensions for the human brain; error not plotted.", call.=FALSE, immediate.=TRUE)
                    }
                }
        ...
        fit.stats <- c(MAE=mean(abs(back_data$target-back_data$shifted)), suppressWarnings(ks.test(back_data$target, back_data$shifted)$statistic))
        ...
        if(save.data == TRUE){
            if(mosaic == TRUE){
                ...
                cat("Complete. List contains 1: the shifted raster, 2: the harmonized mosaic, 3: the fitted model, 4: bulk shift statistics, 
                    5: training data and predictions \n")
                result <- list(shifted=shifted, mosaic=mosaic, model=error.model, fit.stats=fit.stats, data=back_data)
                
            } else {
                cat("Complete. List contains 1: the shifted raster, 2: the fitted model, 3: bulk shift statistics, 4. training data and predictions \n")
                result <- list(shifted=shifted, model=error.model, fit.stats=fit.stats, data=back_data)
            }
            ...
            if(mosaic == TRUE){
                ...
                cat("Complete. List contains 1: the shifted raster, 2: the harmonized mosaic, 3: the fitted model, 4: bulk shift statistics \n")
                result <- list(shifted=shifted, mosaic=mosaic, model=error.model, fit.stats=fit.stats)
                
            } else {
                cat("Complete. List contains 1: the shifted raster, 2: the fitted model, 3: bulk shift statistics \n")
                result <- list(shifted=shifted, model=error.model, fit.stats=fit.stats)
            }
        }
        
        cat("\nFitted MAE =",fit.stats[1], "; K-S statistic D =",fit.stats[2], "\n")
        return(result)
    }